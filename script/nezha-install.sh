...
install_arch(){
    ...
    case $input in
        [yY][eE][sS] | [yY])
            useradd -m agent
            sed -i "$ a\agent ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers
            sudo -iu agent bash -c 'gpg --keyserver keys.gnupg.net --recv-keys BE22091E3EF62275;
            ...
            sed -i '/agent/d'  /etc/sudoers && sleep 30s && killall -u agent&&userdel agent
...
install_agent() {
    ...
    mv nezha-agent $NZ_AGENT_PATH/agent &&
...
EOF
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        echo "@reboot ${NZ_AGENT_PATH}/agent -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} ${args}" >> /etc/crontabs/root
        crond
    fi
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        nohup ${NZ_AGENT_PATH}/agent -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} ${args} >/dev/null 2>&1 &
    fi
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        grep -q 'agent' /var/spool/cron/crontabs/root && sed -i "/agent/d" /var/spool/cron/crontabs/root
        pkill agent
    fi
