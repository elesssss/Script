...
if readlink /proc/$$/exe | grep -q "dash"; then
	echo -e "${Error}该安装程序需要用 "bash "而不是 "sh "运行。"
	exit
fi
...
if grep -qs "ubuntu" /etc/os-release; then
	...
	echo -e "${Error}该安装程序似乎运行在一个不受支持的发行版上。支持的发行版有 Ubuntu、Debian、AlmaLinux、Rocky Linux、CentOS 和 Fedora。"
	exit
fi

if [[ "$os" == "ubuntu" && "$os_version" -lt 1804 ]]; then
	echo -e "${Error}使用此安装程序需要 Ubuntu 18.04 或更高版本。该版本的 Ubuntu 过于老旧，不受支持。"
	exit
fi

if [[ "$os" == "debian" ]]; then
	if grep -q '/sid' /etc/debian_version; then
		echo -e "${Error}本安装程序不支持 Debian Testing 和 Debian Unstable。"
		exit
	fi
	if [[ "$os_version" -lt 9 ]]; then
		echo -e "${Error}使用此安装程序需要 Debian 9 或更高版本。该版本的 Debian 太旧，不支持。"
		exit
	fi
fi

if [[ "$os" == "centos" && "$os_version" -lt 7 ]]; then
	echo -e "${Error}使用此安装程序需要 CentOS 7 或更高版本。该版本的 CentOS 太旧，不支持。"
	exit
fi

# Detect environments where $PATH does not include the sbin directories
if ! grep -q sbin <<< "$PATH"; then
	echo -e "${Error}$PATH 不包括 sbin。尝试使用 "su - "代替 "su"。"
	exit
fi

if [[ "$EUID" -ne 0 ]]; then
	echo -e "${Error}该安装程序需要以root权限运行。"
	exit
fi

if [[ ! -e /dev/net/tun ]] || ! ( exec 7<>/dev/net/tun ) 2>/dev/null; then
	echo -e "${Error}系统没有可用的 TUN 设备。运行此安装程序前需要启用 TUN。"
	exit
fi
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	if ! hash wget 2>/dev/null && ! hash curl 2>/dev/null; then
		echo -e "${Tip}使用此安装程序需要 Wget。"
		read -n1 -r -p "按任意键安装 Wget 并继续..."
		apt-get update
		apt-get install -y wget
	fi
	echo
	echo -e "${Info}欢迎使用 OpenVPN 安装程序！"
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo
		echo -e "${Info}该服务器位于 NAT 后面。公共 IPv4 地址是什么？"
	...
	echo
	echo -e "${Info}OpenVPN 应使用哪种协议？"
	echo "   1) UDP (推荐)"
	echo "   2) TCP"
	read -p "协议 [1]: " protocol
	until [[ -z "$protocol" || "$protocol" =~ ^[12]$ ]]; do
		echo -e "${Error}$protocol: 选择无效。"
		read -p "协议 [1]: " protocol
	done
	case "$protocol" in
		1|"") 
	...
	until [[ -z "$dns" || "$dns" =~ ^[1-6]$ ]]; do
		echo -e "${Error}$dns: 选择无效。"
		read -p "DNS 服务器 [1]: " dns
	done
	echo
	echo -e "${Info}输入第一个用户名："
	...
	if ! systemctl is-active --quiet firewalld.service && ! hash iptables 2>/dev/null; then
		if [[ "$os" == "centos" || "$os" == "fedora" ]]; then
			...
			echo -e "${Info}还将安装管理路由表所需的 firewalld。"
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	case "$option" in
		1)
			...
			new_client
			echo
			echo -e "${Info}已添加 $client。配置在" ~/"$client.ovpn"
			exit
		;;
		2)
			...
			echo
			echo -e "${Info}选择要删除的用户："
			tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | nl -s ') '
			read -p "用户: " client_number
			until [[ "$client_number" =~ ^[0-9]+$ && "$client_number" -le "$number_of_clients" ]]; do
				echo -e "${Error}$client_number: 选择无效。"
				read -p "用户: " client_number
			done
