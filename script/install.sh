...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -s https://api.github.com/repos/FranzKafkaYu/x-ui/releases/latest | grep -oP '"tag_name": "\K([^"]+)')
    ...
    last1_version=$(curl -s https://api.github.com/repos/MHSanaei/3x-ui/releases/latest | grep -oP '"tag_name": "\K([^"]+)')
    wget -N --no-check-certificate -O /root/x-ui-linux-${arch}.tar.gz https://github.com/MHSanaei/3x-ui/releases/download/${last1_version}/x-ui-linux-${arch}.tar.gz
    cd /root && tar zxvf x-ui-linux-${arch}.tar.gz
    cp -f /root/x-ui/x-ui /usr/local/x-ui/
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/RyanY610/logs/main/files/x-ui.sh
    cp -f /usr/bin/x-ui /usr/local/x-ui/x-ui.sh
    chmod +x /usr/local/x-ui/x-ui /usr/local/x-ui/x-ui.sh /usr/local/x-ui/bin/xray-linux-${arch} /usr/bin/x-ui
